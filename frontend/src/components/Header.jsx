            ...
            <div className="flex justify-between items-center ml-10 mr-10">
                ...
                <div id='headerevent' className="items-center space-x-4">
                    <nav style={{ display: 'flex', gap: '5px' }} className="space-x-4 ">
                        ...
                        <NavDropdown title="ETKİNLİKLER" id="basic-nav-dropdown" style={{ color: 'white', fontSize: "20px", paddingTop: "4px" }} className='font-medium z-40 '>
                            {categories.map((category) => (
                                <NavDropdown.Item className='mt-2' key={category.id}>
                                    <Link to={`/categories/${category.id}`} style={{ color: '#3730a3', textDecoration: 'none', fontSize: "18px", }} className='font-bold '><ul className='list-disc'><li>{category.title}</li></ul> </Link>
                                </NavDropdown.Item>
                            ))}
                        </NavDropdown>
                    ...
                    <div className="relative flex items-center mr-3">
                        ...
                        >
                            <RiShoppingBasketFill
                                className={`text-3xl mr-1 ${isHovered ? 'text-blue-300' : 'text-white '} animate-bounce mt-2 `}
...
{/* 
                    ...
                    <Link
                        to={'/login'} 
                        className='text-indigo-700 p-3 no-underline rounded-full bg-white text-xl flex flex-row items-center  hover:bg-white hover:ease-out hover:scale-105 transition duration-500 font-normal hover:ring-2 hover:ring-blue-500'
                    >
