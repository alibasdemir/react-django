...
function Header() {
    ...
    useEffect(() => {
        const fetchCategories = async () => {
            try {
                const response = await fetch('https://fakirherif.pythonanywhere.com/categories');
                if (response.ok) {
    ...
    const handleSearch = async () => {
        ...
        try {
            const response = await fetch(`https://fakirherif.pythonanywhere.com/search-events/?query=${encodeURIComponent(normalizedTerm)}`);
            if (response.ok) {
                const data = await response.json();
                        ...
                        <>
                            ...
                            <Dropdown>
                                ...
                                <Dropdown.Menu className='border-2 border-indigo-600 z-20 bg-white/25'>
                                    ...
                                    <Dropdown.Item as={Link} to="/userlist">
                                    Kullanıcı Listesi
                                    <span className="absolute w-full h-0.5 bg-blue-600 bottom-0 left-0 transform scale-x-0 group-hover:scale-x-100 transition-transform "></span>
                                    </Dropdown.Item>
